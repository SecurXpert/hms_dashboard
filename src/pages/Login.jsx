...
const Login = () => {
  ...
  const SHEET_URL = 'https://script.google.com/macros/s/AKfycbwYpEsOl-qeREgeusH1P7jlmUreX9rMMDSKrIjaXyQm3LKGrSViwDGmjGmLUDdpq1ChKg/exec'; // Replace with your Google Apps Script web app URL
  ...
  const handleDemoSubmit = (e) => {
    ...
    // Stringify the payload to send as text/plain
    const payload = JSON.stringify(booking);

    fetch(SHEET_URL, {
      method: 'POST',
      // Change to text/plain to avoid preflight CORS
      headers: {
        'Content-Type': 'text/plain;charset=utf-8',
      },
      body: payload,  // Send as string
    })
      .then((res) => {
      ...
      .catch((err) => {
        console.error('Submit error:', err);  // Add logging for debugging
  ...
  const downloadDemoBookingsAsExcel = () => {
    // Add ?action=get if your GAS expects it for fetching data
    const urlWithParams = SHEET_URL + '?action=get';
    fetch(urlWithParams)
      .then((res) => {
        if (!res.ok) throw new Error('Failed to fetch');
        return res.text();  // Use .text() first to handle potential non-JSON
      })
      .then((text) => {
        let data;
        try {
          data = JSON.parse(text);  // Parse if it's JSON string
        } catch {
        ...
        const bookings = data.slice(1).map((r) => ({
          id: r[0],
          name: r[1],
          clinicOrHospital: r[2],
          location: r[3],
          contact: r[4],
          email: r[5],
          slot: r[6],
