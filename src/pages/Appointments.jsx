...
const btnBase =
  "inline-flex items-center justify-center rounded-xl px-4 py-3 sm:px-3.5 sm:py-2 text-base sm:text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-offset-2";
          ...
          <div className="max-w-9xl mx-auto space-y-5">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              ...
              <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                <button
                  className={btnBlue + " w-full sm:w-auto"}
                  onClick={() => setOpenPatientForm(true)}
                >
                  <FiUserPlus className="mr-2" /> New Patient
                </button>
                <button
                  className={btnGreen + " w-full sm:w-auto"}
                  onClick={() => {
                    setNewPatient(null);
                    setOpenCreate(true);
                  }}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 lg:p-5">
              ...
              <div className="mt-4 flex items-center justify-end">
                <button className={btnBlue + " w-full sm:w-auto"} onClick={listAppointments}>
                  Apply Filters
                </button>
              </div>
            </div>
                      ...
                      items.map((it) => (
                        <tr key={it.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3">
                            <span
                              className={badge(
                                isCompleted(it.scheduled_for)
                                  ? "bg-purple-100 text-purple-700"
                                  : isFuture(it.scheduled_for)
                ...
                items.map((it) => (
                  ...
                  >
                    ...
                    <div className="mt-2 grid grid-cols-1 gap-1 text-sm">
                      <K label="Patient" v={`${it.patient_name} (${it.patient_id ?? "-"})`} />
                      <K label="Doctor" v={`${it.doctor_name} (${it.doctor_id ?? "-"})`} />
                      <K label="Dept" v={it.department || "-"} />
                      <K label="When" v={fmtDT(it.scheduled_for)} />
                      {/* <K label="Slot" v={it.slot_label} /> */}
                      <K
                        label="Priority"
                        v={
                          <span
                      ...
                      />
                      <K label="Payment" v={it.payment_mode} />
                      <K label="Discount %" v={it.discount_pct ?? 0} />
                      <K label="Reason" v={it.reason || it.notes || "-"} />
                      <K label="Fees" v={`₹ ${it.doctor_fee ?? it.amount_payable ?? "-"}`} />
                    </div>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      ...
                      <button
                        className={btnRed + " w-full"}
                        onClick={() =>
                          setCancelCtx({
                            id: it.id,
                            appointment_no: it.appointment_no || String(it.id),
                            patient_name: it.patient_name,
                            doctor_name: it.doctor_name,
                          })
                        }
  ...
  return (
    <div className="fixed inset-0 z-40">
      ...
      <div className="absolute inset-0 flex items-end sm:items-center justify-center p-0 sm:p-4">
        <div className="w-full sm:max-w-2xl h-[100vh] sm:h-auto sm:max-h-[90vh] rounded-none sm:rounded-2xl border border-gray-200 bg-white shadow-xl flex flex-col">
          {/* Sticky header */}
          <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-2xl bg-blue-600 px-4 sm:px-5 py-3 text-white">
            <h3 className="text-lg sm:text-base font-semibold">
              Appointment #{data.appointment_no || data.id}
            </h3>
            <div className="flex gap-2">
              <button
                className={btnRed + " w-auto"}
                onClick={onCancel}
          ...
          {/* Body (scrollable) */}
          <div className="flex-1 overflow-y-auto p-4 sm:p-5 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
...
function CancelAppointmentModal({ appt, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" />
      <div className="absolute inset-0 flex items-end sm:items-center justify-center p-0 sm:p-4">
        <div className="w-full sm:max-w-3xl h-[100vh] sm:h-auto sm:max-h-[90vh] rounded-none sm:rounded-2xl border border-gray-200 bg-white shadow-2xl flex flex-col">
          {/* Header */}
          <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-2xl bg-red-600 px-4 sm:px-5 py-3 text-white">
            <h3 className="text-lg sm:text-base font-semibold">
          ...
          {/* Footer (sticky) */}
          <div className="sticky bottom-0 z-10 flex items-center justify-end gap-2 border-t border-gray-200 px-4 sm:px-5 py-3 bg-white">
            <button className={btnGray + " w-full sm:w-auto"} onClick={onClose} disabled={saving}>
              Close
            </button>
            <button className={btnRed + " w-full sm:w-auto"} onClick={submit} disabled={saving}>
